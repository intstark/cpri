...
//--(时间精度是5us,时间长度是5ms，每个slot不是严格的125us,每5个是0.5ms-第一个大于125us,其余三个小于125us)
...
//1slot=0.125ms 
	...
	localparam  TDD_3_SLOT_VALUE = 18'd61440 -1;//mmw
...
always @ (posedge clk)
	begin
		set_slot_head_sel <= (set_slot_head | set_frame_head_10ms[5]);
		set_symb_head_sel <= (set_symb_head | set_frame_head_10ms[5]);
	end
...
always @ (posedge clk)
	begin
      ...
      o_slot_head        <= set_slot_head_sel&data_valid_region;
      o_symb_head        <= set_symb_head_sel&data_valid_region;
	end	
...
always @ (posedge clk)
    begin
        if(set_frame_head_5ms[3])
            cnt_10us_or_5us_num <= 10'b0;
        else if(timer_vld)
            if(cnt_10us_or_5us_num == max_total)
                cnt_10us_or_5us_num <= 10'b0;
            else 
...
always @ (posedge clk)
    begin
       set_frame_head_5ms_dl <= set_frame_head_5ms[4];
    end 

assign  pulse_10us_or_5us = head_10us_or_5us || set_frame_head_5ms_dl;
